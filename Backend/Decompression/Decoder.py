...
logger = logging.getLogger('LLMPress.Decoder')
...
@with_error_handling(
    context="Binary data decoding",
    handled_exceptions={
        IndexError: DecodingError,
        ValueError: DecodingError,
...
def decode_bytes(data: bytes) -> Tuple[List[Tuple[str,int]], int]:
    ...
    Raises:
        DecodingError: If decoding fails for any reason
    """
    logger.info(f"Decoding {len(data)} bytes of binary data")
    tokens = []

    # Extract the window size from the beginning of the data
    window_size, idx = extract_window_size(data)
    logger.info(f"Extracted window size: {window_size}")
    ...
    while idx < len(data):
        ...
        if new_idx == idx:  # Prevent infinite loops
            logger.warning(f"Decoding stopped at byte {idx} to avoid infinite loop")
            idx += 1
        else:
            idx = new_idx
        tokens.extend(new_tokens)

    logger.info(f"Successfully decoded {len(data)} bytes into {len(tokens)} tokens")
