...
@handle_operation_errors(
    operation_name="Text Chunking",
    fallback_exception=ChunkingError
)
def split_text(text, min_chunk_size=500, max_chunk_size=2000, file_type=None):
    ...
    if len(reconstructed) != len(text):
        error_msg = f"Chunking integrity issue: Original {len(text)} bytes, Reconstructed {len(reconstructed)} bytes"
...
@handle_operation_errors(
    ...
    specific_exceptions={
        ...
        ChunkingError: (ChunkingError, "Error during text chunking: {error_message}")
    },
...
def chunk_file(file_path, min_chunk_size=500, max_chunk_size=2000):
    ...
    # Read the file content with fallback to latin-1 encoding
    ...
    # Detect file type and chunk the content
