...
@with_error_handling(
    context="Text chunking operation",
    handled_exceptions={
...
@with_error_handling(
    context="File chunking operation",
    handled_exceptions={
        UnicodeDecodeError: FileOperationError,
        ChunkingError: None,  # Preserve the original ChunkingError
        Exception: ChunkingError  # Wrap other exceptions in ChunkingError
    }
)
def chunk_file(file_path, min_chunk_size=500, max_chunk_size=2000):
    ...
    if not os.path.exists(file_path):
        raise FileOperationError("File not found", file_path=file_path)

    if not os.path.isfile(file_path):
        raise FileOperationError("Not a file", file_path=file_path)
